// Moved from api-server/src/lib/predictions.ts — runs client-side now
...
function getTimeframe(score: number): string {
  if (score < 25) return "12+ months";
  if (score < 40) return "9-12 months";
  if (score < 55) return "6-9 months";
  if (score < 70) return "3-6 months";
  if (score < 85) return "1-3 months";
...
export function analyzePredictions(
  ...
  const avgExercise = habitLogs.length > 0
    ? habitLogs.reduce((s, l) => s + l.exerciseMinutes, 0) / habitLogs.length : 30;
  const avgStress = habitLogs.length > 0
    ? habitLogs.reduce((s, l) => s + l.stressLevel, 0) / habitLogs.length : 5;
  const avgWater = habitLogs.length > 0
    ? habitLogs.reduce((s, l) => s + l.waterGlasses, 0) / habitLogs.length : 6;
  const avgSmoking = habitLogs.length > 0
    ? habitLogs.reduce((s, l) => s + l.smokingCigarettes, 0) / habitLogs.length : 0;
  const avgAlcohol = habitLogs.length > 0
    ? habitLogs.reduce((s, l) => s + l.alcoholDrinks, 0) / habitLogs.length : 0;
  const avgScreenTime = habitLogs.length > 0
    ? habitLogs.reduce((s, l) => s + l.screenTimeHours, 0) / habitLogs.length : 4;
  const avgMeditation = habitLogs.length > 0
    ? habitLogs.reduce((s, l) => s + l.meditationMinutes, 0) / habitLogs.length : 0;
  ...
  if (sleepLogs.length > 0 || habitLogs.length > 0) {
    overallHealthScore = Math.max(0, Math.min(100, Math.round(100 - avgRisk)));
  }
